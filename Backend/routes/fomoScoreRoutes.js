...
router.post('/score', async (req, res) => {
  try {
    ...
    try {
      ...
      // If privyId is provided, update the user's overall score
      let totalScoreResult = null;
      if (privyId) {
        try {
          totalScoreResult = await dataService.updateFomoScore(privyId, fomoScore);
          console.log('Updated total score with FOMO score for privyId:', privyId);
        } catch (err) {
          console.error('Error updating total score:', err);
          // Continue anyway, just won't update the total score
        }
      }
      ...
      return res.json({ 
        score: fomoScore,
        did: userDid,
        totalScore: totalScoreResult ? totalScoreResult.totalScore : null,
        data: {
